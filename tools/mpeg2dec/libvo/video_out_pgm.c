				 ...
				 uint8_t * const * buf)
{
    ...
    //char toto[] = "# Je suis un commentaire pgm";
    //instance->writer (instance, toto, strlen(toto));
		      ...
		      unsigned int chroma_height, vo_setup_result_t * result)
{
    ...
    sprintf (instance->header, "P5\n%d %d\n255\n#ips:%d rff:%d tff:%d prog:%d\n", 2 * chroma_width,
	     height + chroma_height, cadence_ips, is_rff, is_tff, is_prog);
    result->convert = NULL;
    return 0;
}
